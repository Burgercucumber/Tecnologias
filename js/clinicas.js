...
function actualizarModalPago(clinica) {
  const payClinicName = document.getElementById('pay-clinic-name');
  const payLocation = document.getElementById('pay-location');
  const payProcAmount = document.getElementById('pay-proc-amount');
  const payRevAmount = document.getElementById('pay-rev-amount');
  const payTotalAmount = document.getElementById('pay-total-amount');
  const payBtnAmount = document.getElementById('pay-btn-amount');

  if (payClinicName) payClinicName.textContent = clinica.nombre;
  if (payLocation) payLocation.textContent = `${clinica.ciudad}, ${clinica.barrio}`;
  if (payProcAmount) payProcAmount.textContent = formatearCOP(clinica.precio_proc);
  if (payRevAmount) payRevAmount.textContent = formatearCOP(clinica.precio_rev);
  if (payTotalAmount) payTotalAmount.textContent = formatearCOP(clinica.precio_total);
  if (payBtnAmount) payBtnAmount.textContent = `Pagar ${formatearCOP(clinica.precio_total)}`;
}
